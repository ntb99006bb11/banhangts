	...
	class WP_Bootstrap_Navwalker extends \Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ('mega_menu' == $item->object) {
				$megamenu_output      = nest_render_content( $item->object_id );
				$output         .= '<li>' . $megamenu_output;
			}
			else{
			if ( isset( $args->item_spacing ) && 'discard' === $args->item_spacing ) {
				$t = '';
			...
			if ($hasMegaMenu) {
				$classes[] = 'dropdown_full position-static mega_menu';
			}
			 ...
			 * @param int              $depth   Depth of menu item. Used for padding.
			 */
			$id = apply_filters( 'nav_menu_item_id', 'menu-item-' , $item, $args, $depth );
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
				// For items in dropdowns use .dropdown_item instead of .nav-link.
				if ( $depth > 0 ) {
					$atts['class'] = 'dropdown_item nav_link';
				} else {
					$atts['class'] = 'nav_link';
				}
			}
			...
			if ( '' !== $linkmod_type ) {
				...
				// With no link mod type set this must be a standard <a> tag.
				$item_output .= '<i class="fi-rs-angle-down"></i></a>';
			}
		...
		public static function fallback( $args ) {
			...
			$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( ' ', 'steelthemes-nest' ) . '">' . esc_html__( '', 'steelthemes-nest' ) . '</a></li>';
			$fallback_output .= '</ul>';
			...
			if ( array_key_exists( 'echo', $args ) && $args['echo'] ) {
				...
				echo wp_kses($fallback_output , wp_kses_allowed_html('post'));
			} else {
		...
		private function separate_linkmods_and_icons_from_classes( $classes, &$linkmod_classes, &$icon_classes, $depth ) {
			...
			foreach ( $classes as $key => $class ) {
				...
				if ( preg_match( '/^disabled|^sr-only/i', $class ) ) {
					...
					unset( $classes[ $key ] );
				} elseif ( preg_match( '/^dropdown-header|^dropdown-divider|^dropdown_item-text/i', $class ) && $depth > 0 ) {
					/*
			...
			if ( ! empty( $linkmod_classes ) ) {
				foreach ( $linkmod_classes as $link_class ) {
					if ( ! empty( $link_class ) ) {
						...
						} elseif ( 'dropdown_item-text' === $link_class ) {
							$linkmod_type = 'dropdown_item-text';
						}
					}
				}
			}
				...
				foreach ( $linkmod_classes as $link_class ) {
					if ( ! empty( $link_class ) ) {
						...
						if ( 'disabled' === $link_class ) {
							...
							$atts['href'] = '#';
							unset( $atts['target'] );
						} elseif ( 'dropdown-header' === $link_class || 'dropdown-divider' === $link_class || 'dropdown_item-text' === $link_class ) {
		...
		private function linkmod_element_open( $linkmod_type, $attributes = '' ) {
			$output = '';
			if ( 'dropdown_item-text' === $linkmod_type ) {
				$output .= '<span class="dropdown_item-text"' . $attributes . '>';
		...
		private function linkmod_element_close( $linkmod_type ) {
			$output = '';
			if ( 'dropdown-header' === $linkmod_type || 'dropdown_item-text' === $linkmod_type ) {
				/*
				 * For a header use a span with the .h6 class instead of a real
				 * header tag so that it doesn't confuse screen readers.
				 */
